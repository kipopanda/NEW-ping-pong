...
# Background image
background = pygame.image.load("pingiu/background.jpeg")
...
while running:
    ...
    if ball_y_cor <= -5:
        ball_y_cor = -5
        ball_y_cor_change *= -1
        pygame.mixer.Sound("pingiu/wall.wav").play()  # Wall hit sound
    ...
    if (0 <= ball_x_cor <= 32) and (ball_y_cor + 45 >= player_a_y_cor and ball_y_cor - 5 <= player_a_y_cor + 128):
        ball_x_cor = 33
        ball_x_cor_change *= -1
        pygame.mixer.Sound("pingiu/ball hit paddle.wav").play()  # Sound when ball hits player
    ...
    if ball_x_cor < 0:
        ...
        pygame.mixer.Sound("pingiu/ball missed.wav").play()  # Sound when ball missed by player
